...
import { Rotation, SearchResult, MatchFlag } from '@embedpdf/models';
import {
  Action,
  CommandMenuProps,
...
export const panelRenderer: ComponentRenderFunction<PanelProps & { tabsCommandId?: string }> = (
  ...
  const togglePanel = () => {
    ui?.togglePanel({
      id: props.id,
      visibleChild: props.visibleChild || 'thumbnails',
      open: !props.open,
    });
  };
...
export const zoomRenderer: ComponentRenderFunction<ZoomRendererProps> = (
  ...
  const zoomDropdownRef = useRef<unknown>(null);
  ...
  const handleClick = (e: MouseEvent, command: Menu<unknown> | Action<unknown> | null | undefined) => {
    if (command) {

...
class RefWrapper extends Component<{ children: ComponentChildren; ref?: unknown }> {
  render() {
...
export function Tooltip({
  ...
  useEffect(() => {
    ...
    return autoUpdate(reference.current.base, floating.current, () => {
      void computePosition(reference.current!.base, floating.current!, {
        placement: position,
        middleware: [
          offset(8),
          flip(),
  ...
  useEffect(() => {
    ...
    const show = () => {
      clear();
      st.current = setTimeout(() => setVisible(true), delay) as unknown as number;
    };
    const hide = () => {
      clear();
      setVisible(false);
